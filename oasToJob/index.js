...
Object.keys(oasFile.scenarios).forEach(scenario => {
    ...
    if (!fs.existsSync('./h0ck-output')) {
        fs.mkdirSync('./h0ck-output');
    }
    fs.writeFile('./h0ck-output/outputJob-' + scenario + '.yaml', yaml.dump(jobResult), (err) => {
        if (err) {
            throw err;
        }
    ...
    fs.writeFile('./h0ck-output/outputJob-' + scenario + '.json', JSON.stringify(jobResult, null, 2), (err) => {
        if (err) {
            throw err;
        }
        console.log("JSON data is saved.");
